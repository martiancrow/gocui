...
func (v *View) EditDelete(back bool) {
	...
	if back {
		if x == 0 { // start of the line
			...
			// end of the line
			if len(v.lines[v.cy]) == v.cx - 1 {
				deletewitdh = runewidth.RuneWidth(v.lines[v.cy][len(v.lines[v.cy]) - 1].chr)
			}

			

			// v.Write([]byte(fmt.Sprintf("\n%v %v %v\n", len(v.lines[v.cy]), runewidth.RuneWidth(v.lines[v.cy][len(v.lines[v.cy]) - 1].chr), string(v.lines[v.cy][len(v.lines[v.cy]) - 1].chr))))

			// v.Write([]byte(fmt.Sprintf("\n%v\n", v.lines)))

			// if len(v.lines) >= runewidth.RuneWidth(v.lines[y][len(v.lines) - 1].chr) {
			// 	deletewitdh = runewidth.RuneWidth(v.lines[y][len(v.lines) - 1].chr)
			// }

			ch, _ := v.deleteRune(v.cx-deletewitdh, v.cy)
			v.MoveCursor(0-runewidth.RuneWidth(ch), 0, true)
		}
	} else {
...
func (v *View) deleteRune(x, y int) (ch rune, err error) {
	...
	// trim last 0
	if len(v.lines[y]) > 0 && v.lines[y][len(v.lines[y]) - 1].chr == 0 {
